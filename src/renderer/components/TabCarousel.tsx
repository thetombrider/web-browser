...
export function TabCarousel({ tabs, carousel, thumbnails }: TabCarouselProps) {
  ...
  const backdrop = useColorModeValue('blackAlpha.300', 'blackAlpha.700')
  const previewBg = useColorModeValue('gray.100', 'gray.800')
  const previewGradient = useColorModeValue('linear(to-br, gray.100, gray.300)', 'linear(to-br, gray.700, gray.900)')
        ...
        {tabs.map((tab, index) => {
          ...
          return (
            <MotionBox
              ...
              borderRadius="xl"
              border="2px solid"
              borderColor={focused ? 'browsy.accent' : cardBorder}
              bg={cardBg}
              boxShadow={focused ? '0 18px 55px rgba(0,0,0,0.42)' : '0 10px 30px rgba(0,0,0,0.26)'}
              overflow="hidden"
              animate={{
                x: offset * 390,
            ...
            >
              <Box h="100%" bg={previewBg}>
                {thumbnails[tab.id] ? (
                  <Image src={thumbnails[tab.id]} alt="" w="100%" h="100%" objectFit="cover" />
                ) : (
                  <Box h="100%" bgGradient={previewGradient} />
                )}
              </Box>
              <Box position="absolute" left={0} right={0} bottom={0} px={4} py={3} bg="blackAlpha.750" color="white">
                ...
                <Text fontSize="xs" color="whiteAlpha.700" noOfLines={1} mt={1}>{hostname(tab.url)}</Text>
              </Box>
            </MotionBox>
          )
        })}
      </Box>
      <Text position="absolute" bottom="8%" color="whiteAlpha.800" fontSize="sm" letterSpacing="wide">
        Enter to switch · Esc to cancel
      </Text>
      <Text position="absolute" top="8%" color="white" fontSize="lg" fontWeight="600">
